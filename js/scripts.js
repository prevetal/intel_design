...
document.addEventListener('DOMContentLoaded', function () {
	...
	$('body').on('click', '.tabs .btn', function(e) {
		...
		if (!$(this).hasClass('active')) {
			...
			// Project plan
			if ($(this).closest('.rooms').length) {
				let roomId = activeTab.replace('_gallery', '')
	...
	if (locationHash && $('.tabs_container').length) {
		...
		// Project plan
		if (activeTab.closest('.rooms').length) {
			let roomId = activeTab.replace('_gallery', '')
	...
	Fancybox.defaults.tpl = {
		closeButton: '<button data-fancybox-close class="f-button is-close-btn" title="{{CLOSE}}"><svg><use xlink:href="https://timurban.ru/wp-content/themes/raten/images/sprite.svg#ic_close"></use></svg></button>',
	...
	// Project plan
	const projectPlanRoomsSliders = [],
		projectPlanRooms = document.querySelectorAll('.project_plan .swiper')

	projectPlanRooms.forEach((el, i) => {
		el.classList.add('project_plan_s' + i)
		...
		projectPlanRoomsSliders.push(new Swiper('.project_plan_s' + i, options))
	})


	$('.project_plan .image svg *').click(function(e) {
		e.preventDefault()

		let parent = $(this).closest('.tab_content'),
			galleryId = '#' + $(this).attr('id') + '_gallery'

		parent.find('.image svg *').removeClass('active')
		$(this).addClass('active')

		parent.find('.rooms .tabs .btn').removeClass('active')
		parent.find(`.rooms .tabs .btn[data-content="${galleryId}"]`).addClass('active')

		parent.find('.rooms .tab_content').removeClass('active')
		parent.find(galleryId).addClass('active')

		// Move slider
		let slideIndex = parent.find(`.rooms .tabs .btn[data-content="${galleryId}"]`).closest('.swiper-slide').index()

		projectPlanRoomsSliders[$(this).closest('svg').data('slider-index')].slideTo(slideIndex, 500)
	})
})
